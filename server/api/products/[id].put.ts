...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    await conn.execute(
      `UPDATE products SET
        name=?, sku=?, slug=?, short_desc=?, description=?, category_id=?, brand=?, status=?,
      ...
      [name, sku, slug, shortDesc, description, categoryId, brand, status,
       ...
       JSON.stringify(images), JSON.stringify(specs), JSON.stringify(attachments), weight, id],
    )
    await conn.execute('DELETE FROM product_tags WHERE product_id=?', [id])
    for (const tagId of tagIds) {
      await conn.execute('INSERT IGNORE INTO product_tags (product_id, tag_id) VALUES (?, ?)', [id, tagId])
    }
    await conn.commit()
